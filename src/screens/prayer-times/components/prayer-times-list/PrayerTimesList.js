...
export default function PrayerTimesList() {
  ...
  const calculationMethod = useSelector(
    (state) => state.prayerTimes.calculationMethod
  );
  ...
  const apiKey = process.env.EXPO_PUBLIC_TIMEZONE_FROM_COORDINATES_API_KEY;

  // Update the timezone if coordinates changed then trigger the update of prayer and sunnah times
  useEffect(() => {
    const updateTimeZone = async () => {
      const timezoneInfo = await getTimeZone(
        apiKey,
      ...
      else assignErrorStatus(timezoneInfo.errorStatus);
    };

    if (coordinates !== null) updateTimeZone();
    else assignTimeZone(null);
  }, [coordinates]);
    ...
    const getPrayerTimes = async () => {
      if (coordinates && timeZone && calculationMethod && mazhab) {
        const prayerTimesParameters =
          getCalculationMethodParam(calculationMethod);
        prayerTimesParameters.madhab = getMazhabParam(mazhab);
        const date = new Date();
        const currentPrayerTimes = new PrayerTimes(
          coordinates,
          date,
          prayerTimesParameters
        );
      ...
      {prayerTimes && sunnahTimes && timeZone ? (
        <View style={styles.paryerTimesList}>
          <PrayerTimesItem
            prayer={{
              name: "الفجر",
              time: toArabicNumerals(formatTime(prayerTimes.fajr)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "الشروق",
              time: toArabicNumerals(formatTime(prayerTimes.sunrise)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "الظهر",
              time: toArabicNumerals(formatTime(prayerTimes.dhuhr)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "العصر",
              time: toArabicNumerals(formatTime(prayerTimes.asr)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "المغرب",
              time: toArabicNumerals(formatTime(prayerTimes.maghrib)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "العشاء",
              time: toArabicNumerals(formatTime(prayerTimes.isha)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "منتصف الليل",
              time: toArabicNumerals(formatTime(sunnahTimes.middleOfTheNight)),
            }}
          />
          <PrayerTimesItem
            prayer={{
              name: "ثلث الليل الأخير",
              time: toArabicNumerals(
        ...
        <View style={styles.messageContainer}>
          <Text style={styles.text}>
            من فضلك، اضبط الإعدادات حتى يتم عرض مواقيت الصلاة
          </Text>
